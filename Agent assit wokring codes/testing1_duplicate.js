...
function startFFmpegCapture() {
  ...
  try {
    ...
    if (label.includes("B_leg") || label.includes("Bleg")) {
      // Less aggressive for Agent - boost more, denoise less
      audioFilter = 'afftdn=nr=8,highpass=f=80,lowpass=f=4000,volume=20dB';
    } else {
      // Keep current for Customer (already working)
      audioFilter = 'afftdn,anlmdn,highpass=f=120,lowpass=f=3800,volume=12dB';
    }
