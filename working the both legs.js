...
function startRtpCapture(label, rtpPort, uniqueKey) {
  ...
  const wsUrl = `${wsBase}${legPath}?callId=${encodeURIComponent(callData.callIdRaw)}&agentId=${siprecMeta.agentId || ''}`;
...
function startFFmpegCapture() {
  ...
  try {
    // ?? LEG-SPECIFIC AUDIO FILTERING
    let audioFilter;

    if (label.includes("B_leg") || label.includes("Bleg")) {
    // Agent leg
         audioFilter = 'afftdn=nr=2,highpass=f=80,lowpass=f=7000';
    } else {
    // Customer leg
         audioFilter = 'afftdn=nr=1,highpass=f=120,lowpass=f=3800';
    }
    ...
    const ffmpegArgs = [
      ...
      '-vn',
      '-af', audioFilter,  // ?? Use dynamic filter instead of hardcoded
      '-acodec', 'pcm_s16le',
      '-ar', '16000',
      '-ac', '1',
      '-f', 's16le',
      ...
      ffmpegProcess.stdout.on('data', (chunk) => {
        ...
        if (ws && ws.readyState === WebSocket.OPEN) {
          ...
          for (let i = 0; i < chunk.length; i += chunkSize) {
			...
			if (label.includes("B_leg") && !printedAgent) {
               console.log("Agent audio being sent to Vosk...");
               printedAgent = true;
            }
